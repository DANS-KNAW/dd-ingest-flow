...
import nl.knaw.dans.ingest.core.domain.DepositBag;
...
public class DepositIngestTask2 implements TargetedTask, Comparable<DepositIngestTask2> {

    private static final Logger log = LoggerFactory.getLogger(DepositIngestTask2.class);
    ...
    @Override
    public void run() {
        writeEvent(TaskEvent.EventType.START_PROCESSING, Result.OK, null);

        var deposit = depositManager.readDeposit(depositLocation);

        // update bag information with deposit user name
        DepositBag bag = bagManager.readBag(deposit.getBagDir());
        bag.setDataStationUserAccount(deposit.getDepositorUserId());
        bag = bagManager.saveBag(bag);

        // validate the bag (validates local properties, and calls dans bag validator)
        depositValidator.validateDeposit(deposit);
        bagValidator.validateBag(bag);

        // create sha1 manifest if not present
        bag = bagManager.ensureSha1ManifestPresent(bag);
        ...
        try {
            ...
            writeEvent(TaskEvent.EventType.END_PROCESSING, Result.OK, null);
        }
            ...
            writeEvent(TaskEvent.EventType.END_PROCESSING, Result.REJECTED, e.getMessage());
        }
        catch (Throwable e) {
    ...
    @Override
    public void writeEvent(TaskEvent.EventType eventType, Result result, String message) {
    ...
    @Override
    public int compareTo(DepositIngestTask2 depositIngestTask) {
